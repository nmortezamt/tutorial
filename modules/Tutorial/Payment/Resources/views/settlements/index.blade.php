...
@section('content')
    <div class="tab__box">
        <div class="tab__items">
            ...
            <a class="tab__item " href="?status={{ \Tutorial\Payment\Models\Settlement::STATUS_SETTLED }}">تسویه های واریز
                شده</a>
        ...
        <table class="table">
            <thead role="rowgroup">
                <tr role="row" class="title-row">
                    ...
                    <th>مبلغ (تومان )</th>
                    <th>وضعیت</th>
                    @can(\Tutorial\RolePermissions\Models\Permission::PERMISSION_MANAGE_SETTLEMENTS)
                    <th>عملیات</th>
                    @endcan
            ...
            <tbody>
                @foreach ($settlements as $settlement)
                    <tr role="row">
                        ...
                        <td><a
                                href="{{ route('users.info', $settlement->user->id) }}">{{ $settlement->user->name ?? '-' }}</a>
                        </td>
                        ...
                        <td><a
                                href="">{{ $settlement->settled_at ? $settlement->settled_at->diffForHumans() : '-' }}</a>
                        </td>
                        <td><a href="">{{ number_format($settlement->amount) }}</a></td>
                        <td><a href=""
                                class="{{ $settlement->getStatusCssClass() }}">{{ __($settlement->status) }}</a></td>
                        @can(\Tutorial\RolePermissions\Models\Permission::PERMISSION_MANAGE_SETTLEMENTS)
                            <td>
